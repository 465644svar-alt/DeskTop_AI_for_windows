...
• Follow-ups"""
    ...
    def _create_arbitrator_tab(self):
        ...
        self.calibrate_btn = ctk.CTkButton(
            btn_frame, text="Калибровка", height=40,
            corner_radius=10, font=ctk.CTkFont(size=14, weight="bold"),
            fg_color="#3498db", hover_color="#2980b9",
    ...
    def get_role_text(self) -> str:
        ...
        if provider_key and provider_key in self.role_prompts:
            return self.role_prompts[provider_key].get("1.0", "end-1c").strip()
        return ""

    def get_prohibitions_text(self) -> str:
        return self.prohibitions_text.get("1.0", "end-1c").strip()

    def get_tasks_text(self) -> str:
        return self.tasks_text.get("1.0", "end-1c").strip()

    def start_calibration(self):
        if hasattr(self, "calibrate_btn"):
            self.calibrate_btn.configure(state="disabled")
        self.log_admin("=== Calibration started ===")
    ...
    def run_calibration(self):
        for name, provider in self.providers.items():
            provider_key = self._provider_key_from_name(name)
            self._calibration_provider_key = provider_key
            role = self.get_role_text()
            prohibitions = self.get_prohibitions_text()
            tasks = self.get_tasks_text()
            self.calibrate_provider(provider, provider_key, role, prohibitions, tasks)

        self.after(0, lambda: getattr(self, "calibrate_btn", None) and self.calibrate_btn.configure(state="normal"))
        self.log_admin("=== Calibration finished ===")

    def calibrate_provider(self, provider, provider_key: str, role: str, prohibitions: str, tasks: str):
        self.log_admin(f"[{provider.name}] Calibration start")
        if not provider.api_key:
            self.log_admin(f"[{provider.name}] RESULT: ERROR (invalid API key)")
            return

        self.check_datetime(provider)
        self.check_role(provider, role)
        self.check_prohibitions(provider, role, prohibitions)
        self.check_tasks(provider, role, prohibitions, tasks)

    def check_datetime(self, provider):
        prompt = (
            "Return current date and time in ISO 8601 format.\n"
            "Return STRICT JSON:\n"
            "{ \"ok\": true, \"iso\": \"YYYY-MM-DDTHH:MM:SS\" }"
        )
        ...
        self._run_calibration_step(
            ...
            "DATETIME",
            messages,
            self._validate_datetime_response
        )

    def check_role(self, provider, role: str):
        messages = [
            {"role": "system", "content": role},
            {"role": "user", "content": "Confirm your role in one sentence. JSON only."}
        ]
        self._run_calibration_step(
            provider,
            "ROLE",
            messages,
            lambda text: self._validate_role_response(text, role)
        )

    def check_prohibitions(self, provider, role: str, prohibitions: str):
        messages = [
            {"role": "system", "content": role},
            {"role": "system", "content": prohibitions},
            {"role": "user", "content": "List active prohibitions. JSON only."}
        ]
        self._run_calibration_step(
            provider,
            "PROHIBITIONS",
            messages,
            lambda text: self._validate_prohibitions_response(text, prohibitions)
        )

    def check_tasks(self, provider, role: str, prohibitions: str, tasks: str):
        messages = [
            {"role": "system", "content": role},
            {"role": "system", "content": prohibitions},
        ...
        self._run_calibration_step(
            ...
            "TASKS",
            messages,
            self._validate_tasks_response
        )

    def _provider_key_from_name(self, name: str) -> str:
        for provider_name, key, *_ in self.PROVIDER_INFO:
            if provider_name == name:
                return key
        return ""

    def _run_calibration_step(self, provider, step: str, messages: List[dict], validator):
        retries = 2
        for attempt in range(retries + 1):
            self.log_admin(f"[{provider.name}] {step} REQUEST")
            self.log_admin(f"[{provider.name}] {step} PAYLOAD: {messages}")

            response_text, error = self._calibration_request(provider, messages)
            if error:
                self.log_admin(f"[{provider.name}] {step} ERROR: {error}")
                if "timeout" in error.lower() and attempt < retries:
                    self.log_admin(f"[{provider.name}] {step} RETRY {attempt + 1}")
                    continue
                return

            self.log_admin(f"[{provider.name}] {step} RESPONSE: {response_text}")
            ok, reason = validator(response_text)
            if ok:
                self.log_admin(f"[{provider.name}] {step} RESULT: OK")
                return
            self.log_admin(f"[{provider.name}] {step} RESULT: ERROR ({reason})")
            if attempt < retries:
                self.log_admin(f"[{provider.name}] {step} RETRY {attempt + 1}")

    def _calibration_request(self, provider, messages: List[dict], timeout: int = 30):
        if isinstance(provider, AnthropicProvider):
            return self._request_anthropic(provider, messages, timeout)
        if isinstance(provider, GeminiProvider):
            return self._request_gemini(provider, messages, timeout)
        return self._request_openai_style(provider, messages, timeout)

    def _request_openai_style(self, provider, messages: List[dict], timeout: int):
        headers = {
        ...
        if response.status_code != 200:
            return "", self._format_provider_error(response.status_code)
        try:
    ...
    def _request_anthropic(self, provider, messages: List[dict], timeout: int):
    ...
    def _request_gemini(self, provider, messages: List[dict], timeout: int):
        system_text = "\n".join(msg["content"] for msg in messages if msg["role"] == "system").strip()
        contents = []
        for msg in messages:
            if msg["role"] == "system":
        ...
        try:
            response = requests.post(
                f"{provider.base_url}/models/{provider.model}:generateContent?key={provider.api_key}",
                headers={"Content-Type": "application/json"},
                json=data,
                timeout=timeout
            )
    ...
    def _format_provider_error(self, status_code: int) -> str:
        if status_code == 401:
    ...
    def _validate_datetime_response(self, text: str):
        payload = self._extract_json(text)
        if not isinstance(payload, dict):
            return False, "format violation"
        iso_value = payload.get("iso")
        if not iso_value:
            return False, "missing iso"
        try:
    ...
    def _validate_role_response(self, text: str, role: str):
        payload = self._extract_json(text)
        if not isinstance(payload, dict):
            return False, "format violation"
        response_text = " ".join(str(value) for value in payload.values()).lower()
        if "ai model" in response_text or "language model" in response_text or "я модель" in response_text:
            return False, "disallowed role statement"
        keywords = self._extract_keywords(role)
        if keywords and not any(word in response_text for word in keywords):
            return False, "role mismatch"
        return True, ""

    def _validate_prohibitions_response(self, text: str, prohibitions: str):
        payload = self._extract_json(text)
        ...
        keywords = self._extract_keywords(prohibitions)
        flat_text = " ".join(str(item).lower() for item in items)
        if keywords and not any(word in flat_text for word in keywords):
            return False, "no keyword match"
        return True, ""

    def _validate_tasks_response(self, text: str):
        if text.strip() != "OK":
            return False, "format violation"
        return True, ""

    def _extract_keywords(self, text: str, limit: int = 6):
        words = []
        for word in re.findall(r"[A-Za-zА-Яа-я0-9]{4,}", text.lower()):
